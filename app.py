...
<title>MailSweep — Email Cleanup Tool</title>
...
<div class="card" data-section="main" id="sec-main">
  <div class="who">
    <span class="dot"></span>
    <span id="lbl-who">connected</span>
...
<div class="modal-overlay" id="modal">
  <div class="modal">
    <h3>Are you sure?</h3>
    <p id="modal-msg">This will permanently delete <strong id="modal-count">0</strong> emails from <strong id="modal-sender"></strong> across all folders. This cannot be undone.</p>
...
<script>
  ...
  $('btn-login').addEventListener('click', async () => {
    ...
    const res = await api('/api/login', {
      ...
      imap_server: $('inp-imap').value,
    });

    if (res.ok) {
      $('lbl-who').textContent = $('inp-email').value;
      showSection('main');
    } else {
  ...
  $('btn-preview').addEventListener('click', async () => {
    ...
    if (res.ok) {
      ...
      setStatus('preview-status', 'error', '✗ ' + res.error);
    }
  });
  ...
  $('modal-confirm').addEventListener('click', async () => {
    ...
    const res = await api('/api/delete', { sender });
  ...
  async function loadSenders() {
    $('btn-scan').innerHTML = '<span class="spinner"></span> Scanning all folders...';
    $('btn-scan').disabled = true;
    $('sender-list').innerHTML = '<div class="empty">Scanning… this may take a minute for large mailboxes.</div>';
    hide('search-wrap');
    clearStatus('scan-status');

    const res = await api('/api/senders');
    $('btn-scan').textContent = 'Rescan';
    $('btn-scan').disabled = false;

    if (res.ok) {
      allSenders = res.senders;
      $('sender-list').dataset.loaded = '1';
      show('search-wrap');
      renderSenders(allSenders);
    } else {
      setStatus('scan-status', 'error', '✗ ' + res.error);
    }
  }
